...
where the class $C$ is swapped between $A$ and $B$ at each step, while the index $t_C$, initialized to $1$, is increased by 2 after that two jobs of class $C$ are scheduled.
 ...
 It's worth to specify that, if during the process one of the two classes is completely scheduled, $C$ remains fixed to the other class. Also, eventually remaining jobs due to the oddness of $n_A$ and $n_B$, are scheduled in the middle of the sequence in a greedy way taking into account \refe{BAWCT}.\\
