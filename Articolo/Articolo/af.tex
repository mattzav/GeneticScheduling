...
\item \textit{Bidirectional Avolio and Fuduli generation}. This technique is inspired by the exact algorithm used in \cite{av-fud20} to solve the simplified version of BAWCT. The genotype of each individual is given by scheduling, at each step of the process, two jobs for one of the two classes, inserting one of them in the first available position and the other in the last one. At each step, the class from which the two jobs are taken is switched, while the choice among the two possible assignments of the two jobs is done in a greedy way.
Let $P(N_A)$ and $P(N_B)$ be two permutations of $N_A$ and $N_B$, respectively. The generation process can be then formally summarized into two different phases:
\begin{enumerate}
	\item Let $k = \min\{\lfloor n_A/2 \rfloor, \lfloor n_B/2 \rfloor\}$. In this phase $k$ steps are performed, each of which aimed at scheduling two job for one of the two classes.\\ In particular, for $i=1,\ldots,k$, we set either \be \label{eq1}[i] = P(N_C)_i \mbox{ and } [n_A+n_B-i] = P(N_B)_i\ee or \be \label{eq2}[i] = P(N_B)_i \mbox{ and } [n_A+n_B-i] = P(N_A)_i.\ee Following a greedy approach, among \refe{eq1} and \refe{eq2}, the assignment currently minimizing \refe{BAWCT} is chosen.  
	\item As for the previous technique, this phase is performed whenever $n_A \ne n_B$. If this is the case, remaining jobs for one of the two classes are now accommodated in the middle of the sequence. \\Assume $n_A<n_B$, then, for $i=k+1,\ldots,\max\{n_A,n_B\}$, we set $[i] = P(N_B)_i.$ If $n_A>n_B$, it's enough to substitute $P(N_B)$ with $P(N_A)$. 
\end{enumerate}
...
In particular, given an individual $I$ representing a solution $S$, for each of the $\frac{(n_A+n_B)(n_A+n_B-1)}{2}$ couples of indices $(i,j)$, with $i<j$,  we check if by swapping jobs in position $i$ and $j$ we obtain a decrease in the current value of \refe{BAWCT}. If this is the case, we execute the exchange, we update the current value of \refe{BAWCT} and we iterate the process.\\
