...
Finally, as exit conditions, we used a time limit of 1800 seconds like in [2]. Of course, we stop the execution also if a null upper bound is found since, by the definition of \refe{BAWCT}, it trivially corresponds to an optimal solution for BAWCT.\\ 
...
where the class $C$ is swapped between $A$ and $B$ at each step, while the index $t_C$, initialized to $1$, is increased by 2 after that two jobs of class $C$ are scheduled.
 ...
 It's worth to specify that, if during the process one of the two classes is completely scheduled, $C$ remains fixed to the other class. Also, eventually remaining jobs due to the \textbf{oddness} of $n_A$ and $n_B$, are scheduled in the middle of the sequence in a greedy way taking into account \refe{BAWCT}.
\end{itemize}
